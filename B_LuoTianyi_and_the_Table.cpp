...
void solve() {
    ...
    sort(rall(v));
    if(n < m) swap(n, m);
    int largest = v[0], secondLargest = v[1];
    int smallest = v[n * m - 1], secondSmallest = v[n * m - 2];
    int ans1 = largest * (m * n - 1) - secondSmallest * (m - 1) - smallest * (n - 1) * m;
    int ans2 = largest * (n - 1) * m + secondLargest * (m - 1) - smallest * (m * n - 1);
    cout << max(ans1, ans2) << "\n";
}
