...
ll queryTree(int node, int st, int end, int l, int r) {
    if (r < st || end < l) return INT_MAX; // no overlap
    if (l <= st && end <= r) return Tree[node]; // complete overlap
    int mid = (st + end) / 2;
    return min(queryTree(2 * node, st, mid, l, r) ,
