...
int dr[] ={-1,0,1,0};
int dc[] ={0,1,0,-1};
bool check(int r,int c,int n,int m,vector<vector<char>>& grid){
...
void solve() {
        ...
        rep(j,0,m){
            if(grid[i][j]=='B'){
                rep(k,0,4){
                    int nr = i + dr[k];
                    int nc = j + dc[k];
                    if(check(nr,nc,n,m,grid) && grid[nr][nc] == 'G'){
                        cout<<"No\n";
                        return ;  
                    }
                    if(check(nr,nc,n,m,grid) && grid[nr][nc]=='.'){
                        grid[nr][nc] = '#';
                    }
    ...
    while(!q.empty()){
        ...
        rep(k,0,4){
            int nr = r + dr[k];
            int nc = c + dc[k];
            if(check(nr,nc,n,m,grid) && !vis[nr][nc] && grid[nr][nc] != '#' ){
                q.push({nr,nc});
                vis[nr][nc]=1;
            }
        }
