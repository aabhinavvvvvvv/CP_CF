...
void solve() {
    ...
    queue<tuple<int, int, int, int>> q; // (row, col, dis, state)
    ...
    while(!q.empty()){
        ...
        for(int i = 0; i < 4; ++i){
            ...
            if(grid[nr][nc] == 'o' && nstate == 1) continue;
            if(grid[nr][nc] == 'x' && nstate == 0) continue;
            if(grid[nr][nc] == '?') nstate = 1 - state; 
            if(!visited[nr][nc][nstate]){
                visited[nr][nc][nstate] = true;
                q.push({nr, nc, dist + 1, nstate});
            }
        }
