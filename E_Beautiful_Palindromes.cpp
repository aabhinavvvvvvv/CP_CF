...
void solve() {
    ...
    set<int> notpresent;
    for (int i = 1; i <= n; ++i)
        if (mp[i] == 0) notpresent.insert(i);

    auto printPattern = [&](vi &f) {
        rep(i, 0, k) cout << f[i % 3] << " ";
        cout << "\n";
    };

    if (notpresent.empty()) {
        vi f = {v[n - 3], v[n - 2], v[n - 1]};
        printPattern(f);
    } 
    else if (sz(notpresent) == 1) {
        vi f;
        if (v[n - 1] == v[n - 2])
            f = {*notpresent.begin(), v[n - 3], v[n - 2]};
        else
            f = {*notpresent.begin(), v[n - 2], v[n - 1]};
        printPattern(f);
    } 
    else {
        vi f = {*notpresent.begin(), *notpresent.rbegin(), v[n - 1]};
        printPattern(f);
    }
}
