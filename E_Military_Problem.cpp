 ...
 */
void dfs(int node, int par, unordered_map<int,int>& mp, vector<int>& path, vector<int>& subsz, vector<vector<int>>& adj) {
    mp[node] = sz(path);
    path.pb(node);

    for (auto x : adj[node]) {
        if (x == par) continue;
        dfs(x, node, mp, path, subsz, adj);
    }
    subsz[node] = sz(path) - mp[node];
}
