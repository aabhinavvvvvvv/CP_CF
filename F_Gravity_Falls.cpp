 ...
 */
int check(vi& a, vi& b, int st){
    for(int i = st; i < min(a.size(), b.size()); i++){
    ...
    while(len < maxlen){
        ...
        for(int i = 0; i < n; i++){
            ...
            if(newarridx == -1){
                ...
                int better = check(v[i], v[newarridx], len);
                if(better == 1) newarridx = i;
            }
        }
        for(int a = len; a < v[newarridx].size(); a++){
            ans.pb(v[newarridx][a]);
        }
        len = v[newarridx].size();
    }
